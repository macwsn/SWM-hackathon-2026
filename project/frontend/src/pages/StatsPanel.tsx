  ...
  return (
    <div className="h-screen bg-brutal-darker flex flex-col overflow-hidden font-mono">
      ...
      <div className="flex-1 grid grid-cols-2 grid-rows-2 overflow-hidden min-h-0">
          ...
          {lastFrame ? (
            ...
            <div className="w-full h-full pt-8 flex items-center justify-center bg-grid-light">
              <div className="text-center">
                ...
                <span className="text-brutal-yellow font-black animate-pulse text-sm">Oczekiwanie…</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="border-r-4 border-black overflow-hidden flex flex-col bg-brutal-dark">
          <div className="bg-brutal-blue border-b-2 border-black px-3 py-1.5 flex-shrink-0 flex items-center gap-2">
            <span className="text-sm">🤖</span>
            <span className="text-white font-black uppercase text-xs">GEMINI LIVE — OPISY RUCHU</span>
            <span className="ml-auto bg-white/20 text-white text-xs font-bold px-1.5 py-0.5 rounded">{geminiItems.length}</span>
          </div>
          <div className="flex-1 overflow-y-auto p-2 flex flex-col-reverse gap-1.5 bg-grid-light">
            {geminiItems.length === 0 && (
              <div className="flex items-center justify-center h-full">
                <span className="text-gray-500 text-xs font-bold uppercase animate-pulse">Oczekiwanie na opisy…</span>
              </div>
            )}
            {[...geminiItems].reverse().map((item, i) => (
              <div key={i} className={`flex flex-col gap-0.5 p-2 border border-white/10 ${i === 0 ? 'bg-brutal-blue/20 border-l-4 border-l-brutal-blue' : 'bg-white/5'}`}>
                ...
                <span className="text-white/90 text-xs font-bold leading-snug">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="overflow-hidden flex flex-col bg-brutal-dark">
          <div className="bg-brutal-orange border-b-2 border-black px-3 py-1.5 flex-shrink-0 flex items-center gap-2">
            <span className="text-sm">⚡</span>
      ...
      <div className="flex-shrink-0 bg-black border-t-4 border-brutal-pink px-4 py-1 flex items-center justify-between">
        ...
        <span className="text-brutal-pink/50 text-xs font-bold">BLIND ASSIST — DIAGNOSTICS</span>
      </div>
    </div>
  )
}
